...
static Item *deque=NULL;

void queueInit(){
 deque=malloc(SIZE*sizeof(Item));
}
...
void pushDeque(Item a){
 do{
  printf("Inserisci:\n1- per inserire l'elemento in cima\n0- per inserire l'elemento in coda\n");
  scanf("%ld",&flag);
 }while((flag)&&(flag!=1));
 if(!N||N<SIZE){
  if(flag){
...
void shiftRight(){
 Item comodo;
 for(int dim=N+1,i=top; dim>0; dim--){
  comodo=deque[(i+1)%SIZE];
  deque[(i+1)%SIZE]=deque[i];
...
void display(){
 ...
 if(!N)
  ...
  for(int i=N; i>0; i--){
   #ifdef intero
    printf(" %d -> ",deque[point]);
    #else
     #ifdef virgola
      printf(" %f -> ",deque[point]l);
     #else
       printf(" %c -> ",deque[point]);   
    #endif
   #endif
   point=(point+1)%SIZE;
  }
